...
and the two programs used for running a shell (usually back then, the Bourne Shell) on, or copying
files to, a remote machine were named `rsh` and `rcp`, think (`r`)emote `sh` and `cp`, respectively.

However, anyone could watch the unencrypted network traffic, which
that meant that villains could
